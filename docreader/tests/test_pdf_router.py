...
class PDFRouterIntegrationTest(unittest.TestCase):
    ...
    def test_pdfium_parsing_is_serialized_under_concurrent_uploads(self):
        """Regression for PDFium process-global state corruption.

        Some valid PDFs opened fine in isolation but failed with
        "PDFium: Data format error" when multiple uploads were parsed by
        docreader gRPC worker threads at the same time. The parser must
        serialize every PDFium touch behind the global lock.
        """
        candidate_paths = [
            os.path.join(
                os.path.expanduser("~"),
        ...
        if len(fixtures) < 2:
            self.skipTest("local PDF concurrency fixtures not available")

        def parse_one(item):
            name, content = item
            doc = PDFParser(file_name=name, file_type="pdf").parse_into_text(content)
            self.assertGreater(doc.metadata.get("page_count", 0), 0)
            return doc.metadata.get("page_count")

        work = fixtures * 3
        with ThreadPoolExecutor(max_workers=min(6, len(work))) as pool:
            page_counts = list(pool.map(parse_one, work))

        self.assertEqual(len(page_counts), len(work))
